defmodule SowerWeb.AgentLive.ShowTest do
  ...
  defp create_agent_with_subscription(user, seed_attrs \\ %{}) do
  ...
  test "shows deploy button when subscription has matching seed", %{conn: conn, user: user} do
    %{agent: agent} = create_agent_with_subscription(user)
  ...
  test "clicking deploy triggers deployment and redirects", %{conn: conn, user: user} do
    %{agent: agent, subscription: subscription} = create_agent_with_subscription(user)
