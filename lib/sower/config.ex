defmodule Sower.Config do
  ...
  def load() do
    ...
    put_env(:auth, oidc_redirect_url: ~s"#{public_url}/auth")
    put_env(:listen_address, "127.0.0.1")
    put_env(:listen_port, 4000)
    ...
    put_env(SowerWeb.Endpoint,
      url: [host: host, port: port, scheme: scheme],
      http: [ip: ip_to_inet(fetch_env!(:listen_address)), port: fetch_env!(:listen_port)],
  ...
  defp put_env(config_atom, new_values) when is_atom(config_atom) and is_list(new_values) do
    config =
    ...
    Application.put_env(:sower, config_atom, config, persistent: true)
  end

  defp put_env(config_atom, new_value) when is_atom(config_atom) do
    Application.put_env(:sower, config_atom, new_value, persistent: true)
  end

  defp fetch_env!(config_atom) when is_atom(config_atom) do
    Application.fetch_env!(:sower, config_atom)
  end
