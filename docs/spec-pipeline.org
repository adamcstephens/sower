...
high-stakes secrets pair =signed_by= with =modes: ["head"]=.
...
| push         | The pushed branch's head, with branch filters.                         |
...
8. Declared =secrets= resolve against the org's secret store and must be
   admitted by their access rules for this run; =vm.caches= entries
   resolve against registered cache resources.
